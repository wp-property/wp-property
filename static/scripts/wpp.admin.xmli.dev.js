wpp.strings = wpp.strings || {}, wpp.xmli = jQuery.extend({
    ...
    ready: function() {
        ...
        var schedule_id;
        wpp.xmli.refresh_dom(), jQuery(".wpp_xi_sort_rules").live("click", function() {
            ...
            var import_hash = jQuery("#import_hash").val();
            "" != import_hash ? window.open(wpp.instance.home_url + "/?wpp_manage_pending_images=" + import_hash + "&echo_log=true", "wpp_manage_pending_images") : wpp.xmli.actions_bar_message(wpp.strings.xmli.please_save, "bad", 7e3);
            ...
            if ("" == source_type) return void jQuery("#wpp_property_import_source_type").addClass("wpp_error");
            ...
            jQuery.post(wpp.instance.ajax_url, {
                ...
                jQuery("#wpp_property_import_ajax").html("true" == result.success ? "<div class='updated below-h2'><p>" + result.ui + "</p></div>" : "<div class='updated below-h2'><p>" + wpp.strings.xmli.error_occured + "</p></div>");
            }, "json"));
    ...
    import_build_unique_id_selector: function() {
        ...
        jQuery("span.description", uid_container).html("" == selected_id ? wpp.strings.xmli.select_unique_id : wpp.strings.xmli.unique_id_attribute));
    },
    perform_auto_matching: function() {
        ...
        jQuery("#wpp_import_auto_match").attr("disabled", !0), void wpp.xmli.evaluate_source(!1, wpp.xmli.perform_auto_matching);
        if (jQuery("#wpp_property_import_attribute_mapper .wpp_dynamic_table_row option").each(function() {
            ...
            if (-1 == jQuery.inArray(wpp_like_xml_tag, wpp_all_importable_attributes)) return void (-1 == jQuery.inArray(wpp_like_xml_tag, wpp_all_importable_attributes_labels));
    ...
    evaluate_source: function(object, callback_func, do_not_use_cache) {
        ...
        if (remote_url.length && "" != import_type) {
            ...
            jQuery.post(wpp.instance.ajax_url, params, function(result) {
                ...
                void wpp.xmli.source_check_result(wpp.strings.xmli.evaluation_500_error, "bad")) : (wpp.xmli.source_status("bad"), 
                void wpp.xmli.source_check_result(wpp.strings.xmli.request_error + " " + result.responseText, "bad"));
            });
        }
    },
    ...
    actions_bar_message: function(message, type, delay) {
        ...
        return "" == message || void 0 == message ? void (void 0 != delay ? element.delay(delay).fadeOut("slow") : element.hide()) : (add_class && (element.addClass("wpp_i_ajax_message"), 
        element.addClass(add_class)), element.show(), element.html(message), void (void 0 != delay && element.delay(delay).fadeOut("slow")));
    },
    source_check_result: function(message, type) {
        ...
        void element.hide("")) : ("bad" == type && element.addClass("wpp_i_error_text"), 
        void element.html(message));
    },
    preview_raw_preview_result: function(message, type) {
        var element = jQuery("span.wpp_i_preview_raw_data_result");
        return element.removeClass("wpp_i_error_text"), void 0 === message ? void element.html("") : ("bad" == type && element.addClass("wpp_i_error_text"), 
        void element.html(message));
    },
    source_status: function(status) {
        ...
        "" == status ? void jQuery("#wpp_property_import_source_status").hide() : (jQuery("#wpp_property_import_source_status").show(), 
        ...
        void ("server_error" == status && (jQuery("#wpp_property_import_source_status").addClass("wpp_import_source_bad"), 
    ...
    advanced_option_counter: function() {
        ...
        jQuery("#wpp_property_skip_images").is(":checked") ? wpp.xmli.disable_advanced_option('input[name="wpp_property_import[remove_images]"]') : wpp.xmli.enable_advanced_option('input[name="wpp_property_import[remove_images]"]'), 
        ...
        return jQuery(".wpp_property_import_settings").is(":visible") || 0 == count ? void jQuery("span.wpp.xmli.advanced_option_counter").html("") : void jQuery("span.wpp.xmli.advanced_option_counter").html("( " + count + " " + wpp.strings.xmli.enabled_options + ")");
    },
    ...
    determine_settings: function(response) {
        ...
        wpp.xmli.source_status("" != source_url && "" != source_type ? "ready_to_check" : ""), 
    ...
    run_on_import_ui_display: function(response) {
        ...
        wpp.xmli.import_build_unique_id_selector(), jQuery(".wpp_xi_advanced_setting input[type=checkbox]").each(function() {
            ...
            var wrapper = jQuery(this).closest("li.wpp_xi_advanced_setting");
            return "" != jQuery(this).val() && "0" != jQuery(this).val() ? void jQuery(wrapper).addClass("wpp_xi_enabld_row") : (jQuery(this).val(""), 
            void jQuery(wrapper).removeClass("wpp_xi_enabld_row"));
        }), wpp.xmli.advanced_option_counter();
    }
