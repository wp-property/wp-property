...
<div class="properties_pagination <?php echo $settings['class']; ?> wpp_slider_pagination type-<?php echo $settings['type']; ?>">
  ...
  <?php if ($settings['type'] !== 'loadmore' || ($settings['type'] == 'loadmore' && $settings['class'] !== 'wpp_bottom_pagination')) { ?>
      ...
      <span class="wpp_sorter_options"><span class="wpp_sort_by_text"><?php echo $settings['sort_by_text']; ?></span>
        <?php if ($settings['sorter_type'] == 'buttons') { ?>
          <?php foreach ($sortable_attrs as $slug => $label) { ?>
            <span
              class="wpp_sortable_link <?php echo($sort_by == $slug ? 'wpp_sorted_element' : ''); ?> label label-info"
              sort_order="<?php echo $sort_order ?>" sort_slug="<?php echo $slug; ?>"><?php echo $label; ?></span>
          <?php } ?>
        <?php } elseif ($settings['sorter_type'] == 'dropdown') { ?>
          <select class="wpp_sortable_dropdown sort_by label-info" name="sort_by">
            <?php foreach ($sortable_attrs as $slug => $label) { ?>
              <option <?php echo($sort_by == $slug ? 'class="wpp_sorted_element" selected="true"' : ''); ?>
                sort_slug="<?php echo $slug; ?>" value="<?php echo $slug; ?>"><?php echo $label; ?></option>
            <?php } ?>
          </select>
        <?php } else { ?>
