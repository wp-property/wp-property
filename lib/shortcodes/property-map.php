    ...
    class Property_Map extends \UsabilityDynamics\WPP\Shortcode {
      ...
      static function shortcode_property_map( $atts = false ) {
        ...
        if( is_numeric( $args[ 'property_id' ] ) ) {
          $property = \UsabilityDynamics\WPP\Utility::get_property( $args[ 'property_id' ] );
        }
        ...
        $template_found = \UsabilityDynamics\WPP\Utility::get_template_part( array( "content-single-property-map", "property-map" ), array( WPP_Templates ) );
        if( !$template_found ) {
          return false;
        }
        ob_start();
