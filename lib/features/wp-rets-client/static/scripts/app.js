jQuery(function() {
  ...
  var _wpp_retsci_client = wpp_retsci_client || { timezone: '' };
    ...
    jQuery.getJSON( url, { token: config.site_secret_token }, function( res ) {
      if ( res.ok ) {
        ...
        if ( res.status.lastUpdate ) {
          ...
          try {
            var _date = new Date( parseInt( res.status.lastUpdate ) );
            ...
            jQuery( '.last_update_date', container ).html( formattedTime );
