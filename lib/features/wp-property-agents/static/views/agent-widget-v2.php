<div class='wpp_agent_info_single_wrapper_v2'>
...
//        print_r($slug);
  ...
  if ($fields) foreach ($fields as $slug) {
    ...
    $dt_element = "<label class='wpp_agent_stats_{$slug}'>{$display_fields[$slug]}:</label>";
    //** Make link */
    if (WPP_F::isURL($this_field)) {
      $this_field = "<a class='wpp_agent_attribute_link' href='{$this_field}' title='{$this_field}'>{$display_fields[$slug]}</a>";
      $dt_element = "<label class='wpp_agent_stats_{$slug}'></label>";
    }
    $this_field = do_shortcode($this_field);
    $dl_print_fields[] = apply_filters('wpp_agent_widget_field_' . $slug, "{$dt_element}<span class='wpp_agent_stats_{$slug}'>{$this_field}</span>", $display_fields, $slug, $user_data->ID);
  }

  if (isset($dl_print_fields) && is_array($dl_print_fields)): ?>
    <div class="wpp_agent_info_list clearfix"><?php echo implode('<div class="clearfix"></div>', $dl_print_fields); ?></div>
  <?php endif;
  if (!empty($user_data->full_bio)) {
    $field_full_bio = apply_filters('wpp_agent_widget_field_full_bio', "<div class='wpp_agent_full_bio clearfix'>{$user_data->full_bio}</div>", '', full_bio, $user_data->ID);
    echo $field_full_bio;
  }
  ?>
