jQuery(document).on('wpp.ui.settings.ready', function() {
  ...
  jQuery.each(config.taxonomies, function(slug, data) {
    ...
    var gslug       = _.get(config, ['groups', slug ], '');
    var group       = _.get(wp_properties, ['property_groups', gslug ], '');;
    var rewriteSlug = _.get(data, ['rewrite', slug ], slug);
    ...
    var attributes = {
      ...
      wp_properties                     : wp_properties,
      current_search_input              : _.get(wp_properties, ['searchable_attr_fields', slug ], ''),
    };
