...
?><form method="POST" action="<?php echo class_wpp_feps::get_edit_feps_permalink( $_REQUEST['feps'], 'checkout' ); ?>" class="<?php wpp_css("feps-subpaln-template::form","wpp_feps_form"); ?> wpp_feps_subscription_plan_form">
    ...
    <?php $checked = false; ?>
    <h3><?php _e( 'Choose Subscription Plan', ud_get_wpp_feps()->domain ); ?>:</h3>
          ...
          <div class="<?php wpp_css("feps-subpaln-template::input-content","wpp_feps_input_content"); ?>">
            ...
            <ul class="<?php wpp_css("feps-subpaln-template::info","wpp_feps_subscription_plan_info"); ?>">
              ...
              <li class="<?php wpp_css("feps-subpaln-template::info-element","duration"); ?>">
                ...
                <span><?php printf( "%d %s", $plan_data['duration']['value'], _n( $plan_data['duration']['interval'], $plan_data['duration']['interval'].'s', $plan_data['duration']['value'], ud_get_wpp_feps()->domain ) ); ?></span>
              </li>
              <li class="<?php wpp_css("feps-subpaln-template::info-element","is_featured"); ?>">
                <label><?php _e( 'Featured:', ud_get_wpp_feps()->domain ); ?></label>
                <span><?php ( isset( $plan_data[ 'is_featured' ] ) && $plan_data[ 'is_featured' ] == 'true' ) ? _e( 'Yes', ud_get_wpp_feps()->domain ) : _e( 'No', ud_get_wpp_feps()->domain ); ?></span>
              </li>
              <li class="<?php wpp_css("feps-subpaln-template::info-element","images_limit" . ($form_has_images ? '' : " no_image_field") ); ?>">
                ...
                <span class="description"><?php _e( 'The available amount of images which can be uploaded for the property', ud_get_wpp_feps()->domain); ?></span>
              </li>
            </ul>
          </div>
        </div>
  ...
  <h3><?php _e( 'Subscription Plan Options', ud_get_wpp_feps()->domain ); ?>:</h3>
      ...
      <div class="<?php wpp_css("feps-default-template::input-wrapper","wpp_feps_input_wrapper"); ?>">
        <div class="<?php wpp_css("feps-default-template::input-content","wpp_feps_input_content"); ?>">
          <label>
            ...
            <?php printf( __( 'Enable automatic renewal of Subscription plan for the current %1$s when subscription time is expired.', ud_get_wpp_feps()->domain ), WPP_F::property_label( 'singular' ) ); ?>
  ...
  <ul class="<?php wpp_css("feps-subpaln-template::fields-wrapper","feps_actions feps_user_input_fields"); ?>">
    ...
    <li class="<?php wpp_css("feps-subpaln-template::back-action-wrapper","wpp_feps_input_wrapper back_action"); ?>">
      <input type="button" onclick="window.location.href = '<?php echo class_wpp_feps::get_edit_feps_permalink( $post_id, 'edit' ); ?>';" value="<?php _e('Back', ud_get_wpp_feps()->domain); ?>" class="<?php wpp_css("feps-subpaln-template::back-action","btn feps_action_btn"); ?>" />
    </li>
    <?php endif; ?>
    <li class="<?php wpp_css("feps-subpaln-template::submit-action-wrapper","wpp_feps_input_wrapper submit_action"); ?> ">
      <input type="submit" value="<?php _e('Next', ud_get_wpp_feps()->domain); ?>" class="<?php wpp_css("feps-subpaln-template::submit-action","wpp_feps_submit_form btn feps_action_btn"); ?>">
    </li>
  </ul>
