...
define( 'wpp.admin', function() {
  ...
  return function domReady() {
    ...
    if( top === self ) {
      ...
      //in a frame
      if( typeof window.wpp === 'object' && typeof window.wpp.instance === 'object' && typeof window.wpp.instance.iframe_enabled !== 'undefined' && window.wpp.instance.iframe_enabled === true ) {
    ...
    jQuery.fn.wppGroups = function( opt ) {
      ...
      var groupsBlock = jQuery( opt.groupsBox ), sortButton = jQuery( opt.sortButton ), statsRow = instance.parent().parent(), statsTable = instance.parents( '#wpp_inquiry_attribute_fields' ), close = jQuery( opt.closeButton, groupsBlock ), assign = jQuery( opt.assignButton ), unassign = jQuery( opt.unassignButton ), wrapper = jQuery( opt.groupWrapper ), colorpicker = jQuery( 'input.wpp_input_colorpicker', groupsBlock ), groupname = jQuery( 'input.slug_setter', groupsBlock ), remove = jQuery( opt.removeButton, groupsBlock ), sortButton = jQuery( opt.sortButton ),
