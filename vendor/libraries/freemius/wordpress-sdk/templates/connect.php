...
if (!$fs->enable_anonymous() || $is_pending_activation) {
...
?>">
    ...
    <p><?php
      if ($is_pending_activation) {
        echo $fs->apply_filters('pending_activation_message', sprintf(
                        ...
                        __fs('pending-activation-message', $slug), $first_name, '<b>' . $fs->get_plugin_name() . '</b>', '<b>' . $current_user->user_email . '</b>'
        ));
      } else {
        ...
        echo $fs->apply_filters($filter, sprintf(
                        __fs('hey-x', $slug) . '<br>' .
                        __fs($default_optin_message, $slug), $first_name, '<b>' . $fs->get_plugin_name() . '</b>', '<b>' . $current_user->user_login . '</b>', '<a href="' . $site_url . '" target="_blank">' . $site_url . '</a>', '<a href="' . $freemius_site_url . '" target="_blank">freemius.com</a>'
                ), $first_name, $fs->get_plugin_name(), $current_user->user_login, '<a href="' . $site_url . '" target="_blank">' . $site_url . '</a>', '<a href="' . $freemius_site_url . '" target="_blank">freemius.com</a>'
        );
      }
      ?></p>
  </div>
  <div class="fs-actions">
    ...
    <?php else : ?>
      <form method="post" id="freemius-options" action="<?php echo WP_FS__ADDRESS ?>/action/service/user/install/">
        <input type="hidden" name="post_url" value="<?php echo WP_FS__ADDRESS ?>/action/service/user/install/">
...
<?php
// store all html before outputting
global $freemius_optin_slide;
include_once (WPP_Path . 'static/splashes/install.php');
$freemius_optin_slide = ob_get_clean();
$WPP_setup_assistant = new WPP_Setup_Assistant();
$WPP_setup_assistant->render_setup_page($freemius_optin_slide);
?>